## This code is based on Niek Sanders twitter data pull. Due to migration from twitter
## API 1.0 to 1.1, the original code didn't work anymore and I had to update/migrate it to 
## API 1.1
## All my changes commented with ## 

## original header of Sanders Twitter Sentiment Corpus
...
#
# Right now we use unauthenticated requests, which are rate-limited to 180/hr.
...
#
import csv, getpass, json, os, time, urllib, twitter, sys
...
api = twitter.Api(consumer_key=twitterCredentials.CONSUMER_KEY,
                      ...
                      access_token_key=twitterCredentials.OAUTH_TOKEN,
                      access_token_secret=twitterCredentials.OAUTH_SECRET)
...
def download_tweets( fetch_list, raw_dir ):
    ...
    for idx in range(0,len(fetch_list)):
        ...
        ## pull data !! mds !! I need to migrate to API v1.1. https://dev.twitter.com/docs/api/1.1/overview.
        #url = 'http://api.twitter.com/1.1/statuses/show/'+ item[2] + ".json"
        #urllib.urlretrieve( url, raw_dir + item[2] + '.json' )
        try:
        	tweet = api.GetStatus(item[2])
        	with open(raw_dir + item[2] + '.json', "w") as textFile:
        		textFile.write(str(tweet))
        except Exception, e:
        	print "!!! error downloading #%s" % item[2] 
